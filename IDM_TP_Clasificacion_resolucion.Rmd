...
```{r tabla-cortes}
cortes_a <- data.frame(
  Intervalos    = c("60 a 70","70 a 80","80 a 95","95 a 100","100 a 110",
                    "110 a 115","115 a 120","120 a 130","130 a 140"),
...
\[
\text{Exactitud} =
\frac{\text{casos correctamente clasificados por la regla}}
{\text{casos cubiertos por la regla}}
\]

\clearpage

```{r tabla-cobertura-exactitud}
tabla_reglas <- data.frame(
  Regla = c("R1", "R2", "R3", "R4", "R5", "R6"),
  Antecedente = c(
    "Ingreso = Alto",
    "Ingreso = Medio e Historial = Bueno",
    "Ingreso = Medio e Historial = Malo",
    "Ingreso = Bajo e Historial = Malo",
    "Ingreso = Bajo e Historial = Bueno y Deuda = Baja",
    "Ingreso = Bajo e Historial = Bueno y Deuda = Alta"
  ),
  Cobertura = c(4, 3, 2, 3, 1, 1),
  Exactitud = c("100%", "100%", "100%", "100%", "100%", "100%")
)
...
knitr::kable(
  ...
  align = c("l", "l", "c", "c"),
  linesep = "",
  caption = "Cobertura y exactitud de las reglas derivadas del árbol de decisión"
) |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center") |>
...
```{r f-setup-show, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
pkgs <- c("rpart", "rpart.plot", "caret", "C50", "mlbench", "RWeka", "ggplot2")
faltan <- pkgs[!(pkgs %in% installed.packages()[, "Package"])]
if (length(faltan)) install.packages(faltan, repos = "https://cloud.r-project.org")

# El algoritmo PART (Ejercicio 13 d) usa RWeka, que requiere un JDK accesible vía rJava.
# En macOS se resuelve JAVA_HOME de forma automática; en otros sistemas se respeta el definido.
if (Sys.getenv("JAVA_HOME") == "" && file.exists("/usr/libexec/java_home")) {
  jh <- tryCatch(system("/usr/libexec/java_home", intern = TRUE), error = function(e) "")
...
```{r f-helpers, echo=FALSE}
r3 <- function(x) formatC(as.numeric(x), format = "f", digits = 3)
pp <- function(x) formatC(100 * as.numeric(x), format = "f", digits = 1)
n_hojas <- function(m) sum(m$frame$var == "<leaf>")
n_reglas <- function(m) {
  ln <- grep("Number of Rules", capture.output(print(m)), value = TRUE)
  if (length(ln) == 0) return(NA_integer_)
  as.integer(gsub("\\D", "", ln[1]))
}
metrics_test <- function(pred) {
  cm <- confusionMatrix(pred, test$diabetes, positive = "pos")
  c(Exactitud     = unname(cm$overall["Accuracy"]),
...
El conjunto registra 768 observaciones de mujeres de la etnia Pima, descriptas por ocho atributos predictores numéricos (número de embarazos, glucosa, presión arterial, pliegue cutáneo del tríceps, insulina, índice de masa corporal, función de pedigrí diabético y edad) y el atributo de clase `diabetes`, con niveles `neg` y `pos`. La clase está moderadamente desbalanceada: solo el `r pp(mean(df$diabetes == "pos"))` % de los casos corresponde a la clase positiva. Por ese motivo la evaluación no se apoya solo en la exactitud; también se reportan la sensibilidad, la especificidad y el estadístico Kappa, menos sensibles al desbalance.

```{r f-clase, echo=FALSE}
tab  <- table(df$diabetes)
prop <- prop.table(tab)
knitr::kable(
  data.frame(Clase = names(tab), Frecuencia = as.integer(tab),
             Proporción = paste0(formatC(100 * as.numeric(prop), format = "f", digits = 1), " %")),
  booktabs = TRUE, linesep = "", align = c("l", "c", "c"),
  caption = "Distribución de la clase en PimaIndiansDiabetes") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center")
```
...
```{r f-ej11-arbol, echo=TRUE, fig.cap="Árbol de decisión por defecto inducido con rpart (cp = 0.01).", out.width="80%"}
set.seed(2026)
tree_def <- rpart(diabetes ~ ., data = train, method = "class")
rpart.plot(tree_def, type = 2, extra = 104, fallen.leaves = TRUE, box.palette = "BuGn")
```

El árbol se induce con los hiperparámetros por defecto de `rpart`, entre ellos el parámetro de complejidad `cp = 0.01`, que opera como criterio de prepoda: solo admite particiones que reduzcan el error relativo en al menos un uno por ciento. El árbol resultante tiene `r n_hojas(tree_def)` nodos hoja. La glucosa es el atributo seleccionado en la raíz, hecho coherente con el dominio clínico, donde constituye el predictor más informativo de la diabetes.
...
```{r f-ej11-tab-errores, echo=FALSE}
knitr::kable(
  data.frame(
    Conjunto = c("Entrenamiento (resustitución)", "Prueba (generalización)"),
    Exactitud = c(acc_train, acc_test),
    `Tasa de error` = c(1 - acc_train, 1 - acc_test),
    check.names = FALSE),
  booktabs = TRUE, digits = 4, linesep = "", align = c("l", "c", "c"),
  caption = "Error de resustitución frente a error de generalización (árbol por defecto)") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center")
```

*Nota.* Elaboración propia a partir del árbol inducido con `rpart`.

El error de resustitución es de `r pp(1 - acc_train)` % y el error de generalización sube a `r pp(1 - acc_test)` %. La diferencia entre ambos refleja el optimismo del error de entrenamiento: el modelo se ajusta en parte a las particularidades de la muestra de entrenamiento. La indicación `positive = "pos"` es deliberada, porque por defecto `confusionMatrix` toma como clase positiva el primer nivel del factor (`neg`); dado que el objetivo clínico es la detección de diabetes, la clase de interés debe ser `pos`.

*d) Informe la exactitud, la sensibilidad, la especificidad y el estadístico Kappa obtenidos sobre el conjunto de prueba.*

```{r f-ej11-metricas, echo=FALSE}
knitr::kable(
  data.frame(
    Métrica = c("Exactitud", "Sensibilidad", "Especificidad", "Kappa de Cohen"),
    Valor = c(cm_test$overall["Accuracy"], cm_test$byClass["Sensitivity"],
              cm_test$byClass["Specificity"], cm_test$overall["Kappa"])),
  booktabs = TRUE, digits = 4, linesep = "", align = c("l", "c"),
  caption = "Métricas de desempeño sobre el conjunto de prueba (árbol por defecto)") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center")
```

*Nota.* La clase positiva es `pos` (paciente diabética).

Sobre el conjunto de prueba el árbol alcanza una exactitud de `r pp(cm_test$overall["Accuracy"])` % y un Kappa de `r r3(cm_test$overall["Kappa"])`, indicador de una concordancia moderada por encima del azar. La sensibilidad es de apenas `r pp(cm_test$byClass["Sensitivity"])` % frente a una especificidad de `r pp(cm_test$byClass["Specificity"])` %: el modelo detecta bien a las pacientes sanas, pero no identifica a buena parte de las diabéticas reales (falsos negativos). Ese desbalance entre sensibilidad y especificidad es el costo que el árbol por defecto asume para maximizar la exactitud global, y la sensibilidad es la métrica más relevante en un tamiz clínico.
...
```{r f-ej12-completo, echo=TRUE}
n_hojas <- function(modelo) {
  sum(modelo$frame$var == "<leaf>")
}
set.seed(2026)
...
```{r f-ej12-tab-completo, echo=FALSE}
knitr::kable(
  data.frame(
    Conjunto = c("Entrenamiento (resustitución)", "Prueba (generalización)"),
    Exactitud = c(acc_full_train, acc_full_test),
    `Tasa de error` = c(1 - acc_full_train, 1 - acc_full_test),
    check.names = FALSE),
  booktabs = TRUE, digits = 4, linesep = "", align = c("l", "c", "c"),
  caption = "Árbol completo (minsplit = 2, cp = 0): resustitución casi perfecta y mala generalización") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center")
```

*Nota.* Elaboración propia.

Al fijar `minsplit = 2` y `cp = 0` se desactiva toda la prepoda y el árbol crece hasta el límite, con `r n_hojas_full` nodos hoja. La evidencia de sobreajuste es contundente: el error de resustitución cae a `r pp(1 - acc_full_train)` %, casi una memorización del entrenamiento, pero el error de prueba empeora a `r pp(1 - acc_full_test)` %, peor incluso que el del árbol por defecto del Ejercicio 11 (`r pp(1 - acc_test)` %). El crecimiento sin restricción no mejora la generalización: la degrada, porque el árbol modela ruido en lugar de señal.

```{r f-ej12-plot-completo, echo=TRUE, fig.cap="Árbol completo: su tamaño excesivo lo vuelve ilegible, y esa misma complejidad es la causa del sobreajuste.", out.width="95%"}
...
Con `rules = TRUE`, C5.0 primero induce un árbol y luego lo convierte en un conjunto de reglas que después simplifica, con poda de condiciones redundantes y eliminación de reglas que no aportan. El modelo resultante contiene `r n_reglas(c5_rules)` reglas.

*b) Inspeccione el conjunto de reglas generado con summary() e interprete dos de las reglas obtenidas.*

```{r f-ej13-summary, echo=TRUE}
summary(c5_rules)
```
...
metrics_test <- function(predicciones) {
  cm <- confusionMatrix(predicciones, test$diabetes, positive = "pos")
...
```{r f-ej13-tab-c50, echo=FALSE}
knitr::kable(comp_c5, booktabs = TRUE, digits = 4, linesep = "",
  align = c("c", "c", "c", "c"),
  col.names = c("Exactitud", "Sensibilidad", "Especificidad", "Kappa"),
  caption = "Reglas C5.0 frente al árbol rpart sobre el conjunto de prueba") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center", font_size = 11) |>
...
El clasificador de reglas C5.0 supera al árbol `rpart` del Ejercicio 11 en todas las métricas sobre prueba: exactitud `r pp(cm_c5$overall["Accuracy"])` % frente a `r pp(cm_test$overall["Accuracy"])` %, Kappa `r r3(cm_c5$overall["Kappa"])` frente a `r r3(cm_test$overall["Kappa"])` y sensibilidad `r pp(cm_c5$byClass["Sensitivity"])` % frente a `r pp(cm_test$byClass["Sensitivity"])` %. La mejora en sensibilidad es la más valiosa en este problema, porque implica la detección de más diabéticas reales. C5.0 obtiene ese resultado por su poda basada en confianza y por la simplificación del conjunto de reglas, que produce un modelo a la vez compacto y más preciso.
...
```{r f-ej13-tab-final, echo=FALSE}
tabla_final <- rbind(
  `Árbol completo (cp = 0)`    = metrics_test(pred_full_test),
  `Árbol por defecto (Ej. 11)` = metrics_test(pred_test),
  `Árbol podado CV (Ej. 12)`   = metrics_test(predict(tree_pruned, test, type = "class")),
  `Reglas C5.0 (Ej. 13)`       = metrics_test(pred_c5),
  `Reglas PART (Ej. 13 d)`     = metrics_test(pred_part))
knitr::kable(tabla_final, booktabs = TRUE, digits = 4, linesep = "",
  align = c("c", "c", "c", "c"),
  col.names = c("Exactitud", "Sensibilidad", "Especificidad", "Kappa"),
  caption = "Síntesis comparativa de todos los modelos sobre el conjunto de prueba") |>
  kableExtra::kable_styling(latex_options = "HOLD_position", position = "center", font_size = 11) |>
